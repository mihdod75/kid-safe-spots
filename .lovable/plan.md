...
- Migration (existing `devices`/`locations` stay in place, unused):
  ...
  - `beacon_watchers` — `beacon_id`, `user_id`, `label`, unique pair.
  ...
  - `SECURITY DEFINER` `public.is_watching(_beacon, _user)` keeps policies non-recursive. RLS: enrolments admin-only; beacons/positions SELECT for watchers or admins, writes admin-only; watcher rows managed by their own `user_id`. Grants to `authenticated` + `service_role`, no `anon`.
...
- `src/lib/admin.functions.ts`, behind `requireSupabaseAuth` with an in-handler `has_role(userId,'admin')` check before `supabaseAdmin` is loaded: `listEnrollments`, `approveEnrollment`, `rejectEnrollment`, `adminListBeacons`, `adminRenameBeacon`, `adminRotateSecret`, `adminDeleteBeacon`.
- `src/lib/tracking.functions.ts`, behind `requireSupabaseAuth`: `listBeacons`, `getBeacon`, `followBeacon({secretCode, label})`, `unfollowBeacon`, `relabelBeacon`. Demo-seeding in `getTracker` is removed.
- Routes: `src/routes/_authenticated/admin.tsx` (pending requests + beacon management, server-guarded and hidden from non-admins) and a reworked `src/routes/_authenticated/tracker.tsx` with the switcher, follow-by-secret form and realtime channel keyed on the selected beacon.
